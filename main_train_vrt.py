...
from utils.utils_dist import get_dist_info, init_dist, barrier_safe, setup_distributed, get_rank, is_main_process
...
def main(json_path='options/vrt/006_train_vrt_videodeblurring_gopro.json'):
    ...
    parser.add_argument('--launcher', default='pytorch', help='job launcher (ignored, auto-detected)')
    parser.add_argument('--local_rank', type=int, default=0, help='local rank (ignored, use env vars)')
    parser.add_argument('--dist', default=False, help='distributed mode (ignored, auto-detected)')
    ...
    # ----------------------------------------
    # This must happen before parsing options to ensure proper device setup
    setup_distributed()
    
    # ----------------------------------------
    # Parse config and auto-detect distributed mode
    # ----------------------------------------
    opt = option.parse(args.opt, is_train=True)
    # opt['dist'] is already set by option.parse() based on WORLD_SIZE
    ...
    # ----------------------------------------
    # seed (with rank offset for distributed training)
    # ----------------------------------------
    seed = opt['train']['manual_seed']
    if seed is None:
        seed = random.randint(1, 10000)
    
    # Add rank to seed for different random states per process
    # This is important for data augmentation diversity
        ...
        if phase == 'train':
            ...
            if opt['dist']:
                train_sampler = DistributedSampler(train_set, shuffle=dataset_opt['dataloader_shuffle'],
                                                   drop_last=True, seed=seed)  # Use base seed, not seed_rank
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                if gt is not None:
                    ...
                    # Log test metrics to TensorBoard and WANDB
                    if tb_logger is not None:
                        test_metrics = {
                            'psnr': ave_psnr,
                            'ssim': ave_ssim,
