...
class GatedFusionOperator(nn.Module):
    def __init__(
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(in_chans, hidden_chans, kernel_size=1),
    ...
    def forward(self, rgb_feat: torch.Tensor, spike_feat: torch.Tensor) -> torch.Tensor:
        if rgb_feat.dim() == 5:
            ...
            rgb_flat = rgb_feat.reshape(bsz * steps, self.rgb_chans, height, width)
            spike_flat = spike_feat.reshape(bsz * steps, self.spike_chans, height, width)
