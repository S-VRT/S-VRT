...
5. **meta_info 为空**: 若生成的 `data/meta_info/meta_info_GoPro_test_GT.txt`（或训练集对应文件）为空，表示 `test_GT/`、`test_GT_blurred/`（或 `train_GT/`、`train_GT_blurred/`）存在但内部没有PNG帧。可以删除这些空目录后重新运行脚本，或直接使用 `--force` 重新整理：
   ```bash
   python scripts/data_preparation/prepare_gopro_spike_dataset.py --splits train test --force
   ```
   脚本遇到空目录时会输出 `Warning: No frames found ...`，这些目录不会写入 meta_info。
