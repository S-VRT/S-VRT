...
def upsample_convtranspose3d(in_channels=64, out_channels=3, kernel_size=2, stride=2, padding=0, bias=True, mode='2R', negative_slope=0.2):
    assert len(mode)<4 and mode[0] in ['2', '3', '4'], 'mode examples: 2, 2R, 2BR, 3, ..., 4BR.'
    kernel_size = int(mode[0])
    stride = int(mode[0])
    mode = mode.replace(mode[0], 'E')
    up1 = conv(in_channels, out_channels, (1, kernel_size, kernel_size), (1, stride, stride), padding, bias, mode, negative_slope)
    return up1
...
def downsample_strideconv3d(in_channels=64, out_channels=64, kernel_size=2, stride=2, padding=0, bias=True, mode='2R', negative_slope=0.2):
    assert len(mode)<4 and mode[0] in ['2', '3', '4'], 'mode examples: 2, 2R, 2BR, 3, ..., 4BR.'
    kernel_size = int(mode[0])
    stride = int(mode[0])
    mode = mode.replace(mode[0], 'D')
    down1 = conv(in_channels, out_channels, (3, kernel_size, kernel_size), (1, stride, stride),(1, padding, padding), bias, mode, negative_slope)
    return down1
...
# averagepooling + conv (+ relu)
# --------------------------------------------
def downsample_avgpool3d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True, mode='2R', negative_slope=0.2):
    assert len(mode)<4 and mode[0] in ['2', '3'], 'mode examples: 2, 2R, 2BR, 3, ..., 3BR.'
    kernel_size_pool = int(mode[0])
    stride_pool = int(mode[0])
    mode = mode.replace(mode[0], 'PD')
    pool = conv(kernel_size=(1, kernel_size_pool, kernel_size_pool), stride=(1, stride_pool, stride_pool), mode=mode[0], negative_slope=negative_slope)
    pool_tail = conv(in_channels, out_channels, (1, kernel_size, kernel_size), (1, stride, stride), (0, padding, padding), bias, mode=mode[1:], negative_slope=negative_slope)
    return sequential(pool, pool_tail)
