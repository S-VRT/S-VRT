"""Attention and block modules migrated from legacy VRT.

This module provides WindowAttention, TMSA, TMSAG, RTMSA, Stage and helpers.
It's a near-direct extraction tailored to live inside `mmvrt.models.layers`.
"""
...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    """Truncated normal initializer (extracted)."""
    def _norm_cdf(x):
        return (1. + math.erf(x / math.sqrt(2.))) / 2.

    with torch.no_grad():
        low = _norm_cdf((a - mean) / std)
        up = _norm_cdf((b - mean) / std)
...
def get_window_size(x_size: Tuple[int, int, int], window_size: Tuple[int, int, int], shift_size=None):
...
class TMSAG(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size=[6, 8, 8], shift_size=None,
        ...
        self.blocks = nn.ModuleList([
            TMSA(dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size,
                 shift_size=[0, 0, 0] if i % 2 == 0 else self.shift_size, mut_attn=mut_attn,
                 mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                 drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                 norm_layer=norm_layer, use_checkpoint_attn=use_checkpoint_attn,
                 use_checkpoint_ffn=use_checkpoint_ffn, use_sgp=use_sgp,
...
class RTMSA(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint_attn=False, use_checkpoint_ffn=None,
                 use_sgp=False, sgp_w=3, sgp_k=3, sgp_reduction=4):
        super(RTMSA, self).__init__()
        self.residual_group = TMSAG(dim=dim, input_resolution=input_resolution, depth=depth, num_heads=num_heads,
                                    window_size=window_size, mut_attn=False, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                    qk_scale=qk_scale, drop_path=drop_path, norm_layer=norm_layer,
                                    use_checkpoint_attn=use_checkpoint_attn, use_checkpoint_ffn=use_checkpoint_ffn,
                                    use_sgp=use_sgp, sgp_w=sgp_w, sgp_k=sgp_k, sgp_reduction=sgp_reduction)
...
class Stage(nn.Module):
    def __init__(self, in_dim, dim, input_resolution, depth, num_heads, window_size, mul_attn_ratio=0.75,
                 mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path=0., norm_layer=nn.LayerNorm,
                 pa_frames=2, deformable_groups=16, reshape=None, max_residue_magnitude=10,
                 use_checkpoint_attn=False, use_checkpoint_ffn=False, use_sgp=False, sgp_w=3, sgp_k=3, sgp_reduction=4):
        super(Stage, self).__init__()
        # For migration we implement Stage as a wrapper around two TMSAG groups.
        self.reshape = nn.Identity()
        self.residual_group1 = TMSAG(dim=dim, input_resolution=input_resolution,
                                     depth=int(depth * mul_attn_ratio), num_heads=num_heads,
                                     window_size=(2, window_size[1], window_size[2]), mut_attn=True,
                                     mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                     drop_path=drop_path, norm_layer=norm_layer, use_checkpoint_attn=use_checkpoint_attn,
                                     use_checkpoint_ffn=use_checkpoint_ffn, use_sgp=False,
                                     sgp_w=sgp_w, sgp_k=sgp_k, sgp_reduction=sgp_reduction)
        self.linear1 = nn.Linear(dim, dim)
        self.residual_group2 = TMSAG(dim=dim, input_resolution=input_resolution,
                                     depth=depth - int(depth * mul_attn_ratio), num_heads=num_heads,
                                     window_size=window_size, mut_attn=False, mlp_ratio=mlp_ratio,
                                     qkv_bias=qkv_bias, qk_scale=qk_scale, drop_path=drop_path,
                                     norm_layer=norm_layer, use_checkpoint_attn=use_checkpoint_attn,
                                     use_checkpoint_ffn=use_checkpoint_ffn, use_sgp=use_sgp,
                                     sgp_w=sgp_w, sgp_k=sgp_k, sgp_reduction=sgp_reduction)
        self.linear2 = nn.Linear(dim, dim)

    def forward(self, x, flows_backward=None, flows_forward=None):
        x = self.reshape(x)
...
from mmvrt.models.layers.weight_init import trunc_normal_
