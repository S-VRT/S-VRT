"""SpyNet motion component and flow utilities migrated into mmvrt.

This file contains a near-verbatim extraction of the SpyNet implementation
and a companion `flow_warp` utility from the legacy `network_vrt.py`.
Keeping this inside `mmvrt.models.motion` makes the motion functionality
available for incremental migration and testing.
"""
...
try:
    import requests  # optional, only used when downloading pretrained SpyNet
except Exception:
...
def flow_warp(x: torch.Tensor, flow: torch.Tensor, interp_mode: str = 'bilinear',
              padding_mode: str = 'zeros', align_corners: bool = True, use_pad_mask: bool = False) -> torch.Tensor:
    """Warp an image or feature map with optical flow.

    Args:
        x: Tensor with size (n, c, h, w).
        flow: Tensor with size (n, h, w, 2).
    """
    n, _, h, w = x.size()
...
class SpyNet(nn.Module):
    """SpyNet architecture ported from the legacy VRT implementation.

    Args:
        load_path (str): optional path to pretrained SpyNet weights.
        return_levels (list[int]): which pyramid levels to return.
    """

    def __init__(self, load_path: Optional[str] = None, return_levels: list = [5]):
        ...
        if load_path:
            if not os.path.exists(load_path):
                if requests is not None:
                    ...
                    # If requests isn't available, attempt to proceed without pretrained weights.
                    load_path = None

            if load_path and os.path.exists(load_path):
                # load_state_dict expects a dict with 'params' key in legacy checkpoint
    ...
    def process(self, ref: torch.Tensor, supp: torch.Tensor, w: int, h: int, w_floor: int, h_floor: int):
        flow_list = []
        ...
        for level in range(len(ref)):
            ...
            flow = self.basic_module[level](torch.cat([
                ref[level],
                flow_warp(supp[level], upsampled_flow.permute(0, 2, 3, 1), interp_mode='bilinear', padding_mode='border'),
                upsampled_flow
            ], 1)) + upsampled_flow
...
__all__ = ["SpyNet", "flow_warp", "BasicModule"]
...
class SpyNet(nn.Module):
    """SpyNet implementation extracted from legacy code.
    Args:
        load_path (str): path for pretrained SpyNet. Default: None.
        return_levels (list[int]): return flows of different levels. Default: [5].
    """

    def __init__(self, load_path: Optional[str] = None, return_levels: Optional[List[int]] = [5]):
        super(SpyNet, self).__init__()
        self.return_levels = return_levels
        self.basic_module = nn.ModuleList([BasicModule() for _ in range(6)])
        if load_path:
            if not __import__('os').path.exists(load_path):
                import requests
                url = 'https://github.com/JingyunLiang/VRT/releases/download/v0.0/spynet_sintel_final-3d2a1287.pth'
                r = requests.get(url, allow_redirects=True)
                __import__('os').makedirs(__import__('os').path.dirname(load_path), exist_ok=True)
                open(load_path, 'wb').write(r.content)
    ...
    def process(self, ref: torch.Tensor, supp: torch.Tensor, w: int, h: int, w_floor: int, h_floor: int):
        ...
        for level in range(len(ref)):
            ...
            flow = self.basic_module[level](torch.cat([
                ref[level],
                # import local flow_warp lazily to avoid circular imports
                __import__('mmvrt.models.motion.flow_ops', fromlist=['flow_warp']).flow_warp(
    ...
    def forward(self, ref: torch.Tensor, supp: torch.Tensor):
        ...
        h, w = ref.size(2), ref.size(3)
        w_floor = math.floor((__import__('math').ceil(w / 32.0) * 32.0))
        h_floor = math.floor((__import__('math').ceil(h / 32.0) * 32.0))
...
class SpyNet(nn.Module):
    """SpyNet architecture ported from legacy implementation."""
